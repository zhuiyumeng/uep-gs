"""
RTP 服务端 — 将 LOD PLY 文件编码为 RTP 流，通过 UDP 发送给客户端。

用法:
    python -m transport.server \\
        --lod_dir ./transport_data \\
        --prefix room \\
        --sh_degree 3 \\
        --host 0.0.0.0 \\
        --port 5005 \\
...
# 确保可以从上级目录导入 transport 包
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from transport.encoder import RTPEncoder
...
def main():
    parser = argparse.ArgumentParser(description='3DGS RTP Server')
    ...
    meta = encoder.meta
    log.info(
        'Loaded: %d gaussians, %d LODs, %.2f MB, SH degree %d',
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    packets = encoder.encode_frame(seq, ts, args.mtu)
                    total = len(packets)
